...
export async function sendAdminMail({ subject, text, html, attachments = [] }) {
  ...
  if (CONFIG.RESEND_API_KEY) {
    ...
    try {
      ...
      console.log('[MAIL] Processing attachments, count:', attachments.length);
      if (attachments && attachments.length > 0) {
        for (const att of attachments) {
          try {
            console.log('[MAIL] Reading attachment:', att.filename, 'from path:', att.path);
            const fileContent = await fs.promises.readFile(att.path);
            const base64Content = fileContent.toString('base64');
            console.log('[MAIL] Successfully encoded:', att.filename, 'size:', base64Content.length, 'bytes');
            resendAttachments.push({
              filename: att.filename,
              content: base64Content
            });
          } catch (err) {
            console.error('[MAIL] Failed to read attachment:', att.filename, err);
          }
        }
      }
      console.log('[MAIL] Total attachments encoded:', resendAttachments.length);
      ...
      console.log('[MAIL] Sending email with', resendAttachments.length, 'attachments');
