...
async function writeToSpreadsheet(data) {
  try {
    ...
    const values = [[
      ...
      data.answers.q2_rooms || '',                  // Q2 間取り
      data.answers.q3_age || '',                    // Q3 築年数
      data.answers.q4_work_type || '',              // Q4 工事内容
      data.answers.q5_wall_area || '',              // Q5 外壁面積
      data.answers.q6_roof_area || '',              // Q6 屋根面積
      data.answers.q7_wall_material || '',          // Q7 外壁材
      data.answers.q8_roof_material || '',          // Q8 屋根材
      data.answers.q9_wall_condition || '',         // Q9 外壁状態
      data.answers.q10_roof_condition || '',        // Q10 屋根状態
      data.answers.q11_paint_grade || '',           // Q11 塗料グレード
      data.answers.q12_urgency || '',               // Q12 希望時期
    ...
    await sheets.spreadsheets.values.append({
      ...
      range: `${process.env.GSHEET_SHEET_NAME || 'Entries'}!A:U`, // A列からU列まで（21列）
  ...
  try {
    ...
    const htmlContent = `
      ...
      <table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse;">
        ...
        <tr><td style="background-color: #f5f5f5;"><strong>外壁面積</strong></td><td>${data.answers.q5_wall_area || '—'}</td></tr>
        <tr><td style="background-color: #f5f5f5;"><strong>屋根面積</strong></td><td>${data.answers.q6_roof_area || '—'}</td></tr>
        ...
        <tr><td style="background-color: #f5f5f5;"><strong>外壁状態</strong></td><td>${data.answers.q9_wall_condition || '—'}</td></tr>
        <tr><td style="background-color: #f5f5f5;"><strong>屋根状態</strong></td><td>${data.answers.q10_roof_condition || '—'}</td></tr>
...
app.post('/api/submit', upload.array('photos', 10), handleMulterError, async (req, res) => {
  try {
    ...
    console.log('[DEBUG] 受信データ:', { userId, name, phone, zipcode, address1, address2 });
    console.log('[DEBUG] 回答データ:', answers);
    console.log('[DEBUG] 見積りデータ:', estimate);
    console.log('[DEBUG] 写真数:', photos.length);
    ...
    try {
      ...
      console.error('[ERROR] 見積りデータの解析エラー:', error);
      parsedEstimate = { total: 0, breakdown: {} };
    }

    // セッションデータを作成（従来のロジックとの互換性のため）
    const sess = {
      answers: parsedAnswers,
      estimatedPrice: parsedEstimate.total || calcRoughPrice(parsedAnswers)
    };
