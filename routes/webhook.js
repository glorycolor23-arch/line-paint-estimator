...
function resolveLiffUrl(lead) {
  ...
  // ホスト配信されている /liff.html にフォールバック（/liff/index.html ではない）
...
async function handleEvent(event) {
        ...
        if (estimate) {
          ...
          const msg2 = {
            ...
            template: {
              ...
              actions: [
                { type: 'uri', label: '無料で詳細見積もりを依頼する', uri: liffUrl || 'https://line.me' },
              ],
            },
          };
  ...
  // 任意：ユーザーが「現地調査なしで詳細見積もりを依頼する」等の文言を送ったら LIFF を返す
  if (type === 'message' && event.message?.type === 'text') {
    const q = (event.message.text || '').trim();
    if (/詳細見積もり|見積もりを依頼|無料で詳細/i.test(q)) {
      const leadId = await findLeadIdByUserId(userId);
      const liffUrl = resolveLiffUrl(leadId || '');
      const msg = {
        type: 'template',
